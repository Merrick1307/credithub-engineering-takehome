...
def test_full_reversal_of_exact_payoff_reopens_loan(client):
    ...
    assert client.get("/loans/1").json()["status"] == "active"
    assert client.get("/loans/1").json()["outstanding"] == 56_000
...
def test_exact_reversal_redelivery_replays_without_second_balance_change(client):
    ...
    outstanding_after_first = client.get("/loans/1").json()["outstanding"]
    ...
    assert client.get("/loans/1").json()["outstanding"] == outstanding_after_first


def test_partial_reversal_is_quarantined_without_balance_change(client):
    _post_mock(client, _payment("TXN-PARTIAL", 2_000_000))
    outstanding_after_payment = client.get("/loans/1").json()["outstanding"]
    ...
    assert client.get("/loans/1").json()["outstanding"] == outstanding_after_payment
...
def test_refund_consumes_overpayment_without_changing_loan(client):
    ...
    before = client.get("/loans/1").json()
    ...
    assert response.json()["event"]["status"] == "applied"
    assert client.get("/loans/1").json() == before
...
def test_reversal_of_closed_loan_overpayment_preserves_operational_status(client):
    ...
    reversal["customer_id"] = "2"
    response = _post_mock(client, reversal)

    assert response.json()["event"]["status"] == "applied"
    assert client.get("/loans/2").json()["status"] == "cancelled"
